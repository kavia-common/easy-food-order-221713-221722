 ...
 import { shouldUseWebsocket, getWsUrl, getApiBase as getBackendBase } from './runtimeConfig';

 function normalizeBaseUrl(v?: string): string | undefined {
   try {
     if (!v || typeof v !== 'string' || !v.trim()) return undefined;
     // Allow relative API path such as "/api"
     if (v.startsWith('/')) return v;
     // Validate general URL-like string
 ...
 const WS_ENV = normalizeBaseUrl(getWsUrl());
 const API_BASE = normalizeBaseUrl(getBackendBase());
 ...
 function getUserId(): string {
   ...
   const existing = typeof localStorage !== 'undefined' ? localStorage.getItem(LS_USER_ID_KEY) : null;
   if (existing) {
     userIdMemo = existing;
     return existing;
   }
 ...
 function canUseWS(): boolean {
   return typeof window !== 'undefined' && !!(WS_ENV || API_BASE) && shouldUseWebsocket();
 }

 function wsUrlForThread(threadId: string): string | null {
   const base = WS_ENV || (API_BASE ? `${String(API_BASE).replace(/\/*$/, '')}/ws` : '');
   if (!base) return null;
